$(function() {
	...
	function switchV(event) {
		...
		$angleLi.removeClass('active');
		$(this).addClass('active');
		myVideo(DATA.videoList[index]);
	}
	...
	function canplay() {
		...
		video.addEventListener('canplay', function() {
			$loading.hide();
			$videoImg.hide();
			$duration.find('span').eq(1).text(formatTime(this.duration));
		}, false);
		...
		video.addEventListener('timeupdate', function() {
			...
			$duration.find('span').eq(0).text(formatTime(currentTime));
			$duration.find('span').eq(1).text(formatTime(duration));
		}, false);
